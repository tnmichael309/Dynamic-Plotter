...
class information_plotter:
    def __init__(self, is_replay_mode = False, dedicated_file=None):
        ...
        if is_replay_mode:
            global_print("Generating video....")
            current_path = os.path.dirname(os.path.abspath("information_plotter.cfg"))
            ...
            try:
                global_print("Writer generating...")
    ...
    def close(self):
        ...
        if self.timer is not None:
            self.timer.remove_callback(self.close)
            self.timer.stop()
                
        if self.is_replay_mode == False:
            plt.ioff()
            plt.close()
            app.start_replaying(ask=True)
        
            
    def is_first_time(self):
        return self.is_first_time_use
    ...
    def follow(self):
        ...
        global ip_stop_flag, replay_ip_stop_flag,app
        count = 0 
        ...
        while True:
            ...
            if (ip_stop_flag == True and self.is_replay_mode == False) or (replay_ip_stop_flag == True and self.is_replay_mode == True):
                ...
                #plt.close()
                #self.close()
                self.timer = self.fig.canvas.new_timer(interval=self.update_interval*2)
                self.timer.add_callback(self.close)
                self.timer.start()
            ...
            for c, comment in enumerate(self.comment_list):
                ...
                self.annotate_list[ax_index].append(
                    matplotlib.text.Annotation(
                        ...
                        arrowprops=dict(visible=False), fontsize=self.comment_font_size, rotation=353
                    )
                )
...
class Application(Frame):
    def __init__(self,master):
        global basic_unit
        self.frame = Frame.__init__(self,master, width = 6*basic_unit, height = 6*basic_unit)
    ...
    def create_widgets(self):
        self.start_button = Button(self, text="Start Plotting", width=3*basic_unit)
        self.start_button["command"] = self.start_recording
        self.start_button.grid(row = 0, column = 0, sticky=W)
        
        self.replay_button = Button(self, text="Convert Video", width=3*basic_unit)
        self.replay_button["command"] = self.start_replaying
        self.replay_button.grid(row = 0, column = 1, sticky=W)
   
        self.text = Text(self.master, width = 5*basic_unit, height = 3*basic_unit, wrap = WORD)
        ...
        self.vertscroll.grid(column=2, row=1, sticky=N+S)
     
    def start(self):
        # initialzie with log file
        pass
    ...
    def start_recording(self):
        ...
        self.ip = information_plotter()
        
        is_to_continue = True
        
        if self.ip.is_first_time():
            is_to_continue = tmBox.askyesno("Config the Plotter", "Do you want to use default configuration?")
    ...
    def start_replaying(self, ask=False):
        ...
        if ask == False or tmBox.askyesno("Convert Video", "Do you want to save the video ?"):
            ...
            self.replay_ip = information_plotter(is_replay_mode=True, dedicated_file=filename)
            
            if self.replay_ip.is_first_time():
                plt.close()
                self.replay_ip = information_plotter(is_replay_mode=True, dedicated_file=filename)
            
            self.replay_ip.start()
